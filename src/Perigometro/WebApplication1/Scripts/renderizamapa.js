    ...
    $.when.apply($, requisicoes).done(function () {
        ...
        $.each(todos, function (i, elem) {
            ...
            dadosJson.push({
                ...
                'Onibus_Urb': elem.ONIBUS_URB, 'Onibus_Met': elem.ONIBUS_MET, 'Onibus_Int': elem.ONIBUS_INT,
        ...
        console.log('terminou tudo!!', todos);
        console.log('Objeto JSON : ', dadosJson);
        adicionaHeatmap();
    });
...
function adicionaHeatmap() {
    ...
    heatmap.setMap(map);
    json = JSON.stringify({'Acidentes': dadosJson});
    $.ajax({
